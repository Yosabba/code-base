  ...
  const getAllGamePlatforms = (game) => {
    for (let i = 0; i < game.length; i++) {
      for (let j = 0; j < game[i].parent_platforms.length; j++) {
        if (game[i].parent_platforms[j].platform.slug === "playstation") {
          allPlaystationGames.push(game[i]);
        }
      }
    }
  };
  ...
  return (
    ...
    >
      <AnimateSharedLayout type="crossfade">
        ...
        <h1 className="text-5xl ml-2 mobile:text-3xl py-8">Playstation Games</h1>
        {mobileWindowSize ? (
          <Swiper
            slidesPerView={1}
            spaceBetween={5}
